...
class LoanAssistant {
    constructor() {
        ...
        this.loanContractApi = require('../api/kfsApi');
...
Please choose an option that works best for you.`, language),
    ...
    async handleFunctionCall({ name, arguments: args }, messageId) {
        ...
        try {
            ...
            switch (name) {
                ...
                case 'generate_loan_contract':
                    result = await this.generateLoanContract(params);
                    break;
                
                case 'check_loan_contract_status':
                    result = await this.checkLoanContractStatus(params);
                    break;
    ...
    async createLoanAccount(params) {
        try {
            logger.info('Starting loan account creation process', { opportunityId: params.opportunityId });

            // Validate that all required steps are completed
            const validationResult = await this.validatePrerequisites(params.opportunityId);
            if (!validationResult.success) {
                return {
                    success: false,
                    message: `Cannot create loan account: ${validationResult.message}`,
                    pendingSteps: validationResult.pendingSteps
                };
            }

            // Format the submitted data list
            const submittedDataList = this.loanAccountApi.formatSubmittedDataList({
                ...
                additionalDataReferenceId: this.conversationContext.applicationState.additionalDataReferenceId,
                kycReferenceId: this.conversationContext.applicationState.kycReferenceId,
                photoVerificationReferenceId: this.conversationContext.applicationState.photoVerificationReferenceId,
                mobileVerificationReferenceId: this.conversationContext.applicationState.mobileVerificationReferenceId,
                emailVerificationReferenceId: this.conversationContext.applicationState.emailVerificationReferenceId
            });

            // Validate the submitted data list
            this.loanAccountApi.validateSubmittedData(submittedDataList);

            // Submit the opportunity
            const result = await this.loanAccountApi.submitOpportunity(
                params.opportunityId,
                submittedDataList
            );

            if (result.success) {
                // Update conversation context with loan account details
                this.updateContext('create_loan_account', {
                    fenixLoanAccountId: result.data.fenixLoanAccountId,
                    fenixClientId: result.data.fenixClientId,
            ...
            logger.error('Error in loan account creation', error);
            return {
                success: false,
    ...
    async validatePrerequisites(opportunityId) {
        const context = this.conversationContext.applicationState;
        const pendingSteps = [];

        // Checks stored KYC status
        if (!context.kycStatus || context.kycStatus !== 'APPROVED') {
            pendingSteps.push('KYC verification');
        }

        // Checks stored bank account status
        if (!context.bankAccountStatus || context.bankAccountStatus !== 'APPROVED') {
            pendingSteps.push('Bank account verification');
        }

        // Check mandate setup
        if (!context.mandateStatus || context.mandateStatus !== 'APPROVED') {
            pendingSteps.push('Mandate setup');
        }

        // Check agreement status
        if (!context.agreementStatus || context.agreementStatus !== 'COMPLETED') {
            pendingSteps.push('Agreement signing');
        }

        // Check KFS status
        if (!context.kfsStatus || context.kfsStatus !== 'COMPLETED') {
            pendingSteps.push('KFS acceptance');
        }

        // Check mutual fund pledge status
        if (!this.conversationContext.mutualFundContext.pledgeRequestId) {
            pendingSteps.push('Mutual fund pledge');
        }

        if (pendingSteps.length > 0) {
            return {
                success: false,
                message: 'Some required steps are pending',
    ...
    handleLoanAccountCreationResponse(result) {
        if (result.success) {
            return {
                message: `Great news! Your loan account has been created successfully. Your loan account ID is ${result.data.fenixLoanAccountId} and client ID is ${result.data.fenixClientId}. You can now proceed with withdrawal requests.`,
                suggestedActions: [
                    {
                        type: 'VIEW_LOAN_DETAILS',
            ...
            let message = 'There was an issue creating your loan account. ';
            if (result.pendingSteps) {
                message += `The following steps need to be completed first: ${result.pendingSteps.join(', ')}.`;
            } else {
                message += result.message;
            }
            return {
                message: message,
                suggestedActions: result.pendingSteps ? result.pendingSteps.map(step => ({
                    type: `COMPLETE_${step.toUpperCase().replace(/\s+/g, '_')}`,
                    label: `Complete ${step}`
                })) : []
            };
        }
    ...
    async generateLoanContract(params) {
        try {
            logger.info('Initiating loan contract generation', { opportunityId: params.opportunityId });

            // Format the request using the API's helper method
            const requestBody = this.loanContractApi.formatLoanContractRequest({
                ...params,
                kycReferenceId: this.conversationContext.applicationState.kycReferenceId,
                additionalUtilityReferenceId: this.conversationContext.applicationState.additionalDataReferenceId,
                photoUtilityReferenceId: this.conversationContext.applicationState.photoVerificationReferenceId,
                bankAccountReferenceId: this.conversationContext.applicationState.bankAccountReferenceId
            });

            // Generate loan contract
            const result = await this.loanContractApi.generateLoanContract(params.opportunityId, requestBody);

            if (result.success) {
                // Update conversation context with both KFS and Agreement details
                const kfsStep = result.data.steps.find(step => step.utilityType === 'KFS');
                const agreementStep = result.data.steps.find(step => step.utilityType === 'AGREEMENT_SIGN');

                this.updateContext('generate_loan_contract', {
                    loanContractStatus: result.data.status,
                    loanContractSteps: result.data.steps,
                    kfsReferenceId: kfsStep?.utilityReferenceId,
                    kfsStatus: kfsStep?.status,
                    agreementReferenceId: agreementStep?.utilityReferenceId,
                    agreementStatus: agreementStep?.status,
                    redirectionURL: result.data.redirectionURL
                });

                return {
                    success: true,
                    message: "Loan contract generated successfully. Please review and accept both the KFS and Agreement.",
                    data: result.data
                };
            }

            throw new Error(result.error || 'Failed to generate loan contract');

        } catch (error) {
            logger.error('Error in loan contract generation', error);
            return {
                success: false,
                message: `Error generating loan contract: ${error.message}`,
                error: error
            };
        }
    }

    async checkLoanContractStatus(params) {
        try {
            logger.info('Checking loan contract status', { opportunityId: params.opportunityId });

            const result = await this.loanContractApi.getLoanContractStatus(params.opportunityId);

            if (result.success) {
                // Update conversation context with both KFS and Agreement status
                const kfsStep = result.data.steps.find(step => step.utilityType === 'KFS');
                const agreementStep = result.data.steps.find(step => step.utilityType === 'AGREEMENT_SIGN');

                this.updateContext('check_loan_contract_status', {
                    loanContractStatus: result.data.status,
                    loanContractSteps: result.data.steps,
                    kfsStatus: kfsStep?.status,
                    agreementStatus: agreementStep?.status,
                ...
                switch (result.data.status) {
                    case 'IN_PROGRESS':
                        const pendingSteps = result.data.steps
                            .filter(step => step.status === 'IN_PROGRESS')
                            .map(step => step.utilityType === 'KFS' ? 'Key Fact Statement' : 'Loan Agreement');
                        
                        message = `Please complete the following: ${pendingSteps.join(' and ')}`;
                        if (result.data.redirectionURL) {
                            suggestedActions.push({
                                type: 'OPEN_URL',
                                label: 'Review & Accept Documents',
                                url: result.data.redirectionURL
                            });
                        }
                        break;

                    case 'COMPLETED':
                        message = "Great! Both KFS and Loan Agreement have been successfully completed.";
                        suggestedActions.push({
                    ...
                    case 'EXPIRED':
                        message = "The loan contract has expired. Would you like to generate a new one?";
                        suggestedActions.push({
                            type: 'GENERATE_LOAN_CONTRACT',
                            label: 'Generate New Contract'
                        });
                        break;

                    default:
                        message = "Unable to determine loan contract status.";
                }
            ...
            throw new Error(result.error || 'Failed to check loan contract status');

        } catch (error) {
            logger.error('Error checking loan contract status', error);
            return {
                success: false,
                message: `Error checking loan contract status: ${error.message}`,
                error: error
            };
        }
    }

    handleLoanContractResponse(result) {
        if (result.success) {
            ...
            switch (status) {
                case 'IN_PROGRESS':
                    ...
                    message = `Please review and accept the ${pendingSteps.join(' and ')}. You'll be redirected to complete this process.`;
                    if (result.data.redirectionURL) {
                        suggestedActions.push({
                            type: 'OPEN_URL',
                            label: 'Review & Accept Documents',
                ...
                case 'COMPLETED':
                    message = "Excellent! You've successfully completed both the Key Fact Statement and Loan Agreement.";
                    suggestedActions.push({
                        type: 'CREATE_LOAN_ACCOUNT',
                        label: 'Proceed to Create Loan Account'
                    });
                    break;

                case 'EXPIRED':
                    message = "The loan contract has expired. Would you like me to generate a new one for you?";
                    suggestedActions.push({
                        type: 'GENERATE_LOAN_CONTRACT',
                        label: 'Generate New Contract'
                    });
            ...
            return {
                message: `There was an issue with the loan contract: ${result.message}`,
                suggestedActions: [{
                    type: 'RETRY',
                    label: 'Try Again'
                }]
